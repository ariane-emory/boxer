...
fn main() -> io::Result<()> {
  ...
  let process_horiz = Box::new(move |pos: &Point, byte: &u8| {
    ...
    // Use the cloned reference inside the closure
  ...
  // Assume process_file is correctly implemented
  let _ = process_file("./data/data.txt", process_horiz, process_vert);

  // Use the original Rc references, which were not moved, to access the lines
