...
impl LineMaker {
  ...
  pub fn process(
    ...
    // '+----+'.- When a '+' is observed and line_begin is None, the current
    // position is recorded. If line begin is set and the current character
    // is the same as line_body_char, the line is extended. If the current
    // character is a '+', the line is closed and added to the list of lines
    // and line_begin is reset to None. If some other character is observed
    // in the middle (e.g., '+---a---+' the attempt to create a line is
    // abandoned (and line_begin becomes None). A Line must contain at least
    // one line_body character ('++' is not a line).
    if pos.col == 0 {
      println!("         new line!");
